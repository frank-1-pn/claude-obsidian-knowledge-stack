...
#
# Reads $LARK_TMP/lark-<bot>-events.ndjson (Windows %TEMP%) via tail -c +OFFSET -F
...
# Daemon writes NDJSON to Windows %TEMP%. In Git Bash resolve it from $TEMP;
# override with LARK_TMP if your daemon writes elsewhere.
LARK_TMP="${LARK_TMP:-$(cygpath -u "${TEMP:-${TMP:-/tmp}}" 2>/dev/null || echo "${TEMP:-/tmp}")}"
LOG="${LARK_TMP}/lark-${BOT}-events.ndjson"
OFFSET_FILE="${LARK_TMP}/lark-${BOT}-monitor.offset"

# Authorized sender for /compact! trigger
MY_OPEN_ID="ou_REPLACE_WITH_YOUR_OPEN_ID"
