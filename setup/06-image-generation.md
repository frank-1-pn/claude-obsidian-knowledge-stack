...
generates one. The defaults: `gpt-image-2` via an API proxy, all in-image
text in Chinese (for cross-device, no-OCR readability).

## Why this configuration

- **gpt-image-2 over DALL-E 3**: instruction following is much stronger,
...
Drop `config/genimg.py` (template; see below) at `~/.claude/scripts/genimg.py`.

```python
# config/genimg.py — template
"""Generate images via gpt-image-2 through an OpenAI-compatible proxy.
...
API key:
    Reads the first line of $env:USERPROFILE/Desktop/openai_api.txt
    (or override with GENIMG_KEY_FILE env var).

Base URL:
    Default https://api.<your-proxy>.com — override with GENIMG_BASE_URL.

Known gotchas:
    1. Windows system proxy (HTTPS_PROXY=http://127.0.0.1:<port>) intercepts
       proxy traffic. We explicitly install a no-op ProxyHandler so urllib
...
# Bypass any system HTTP/HTTPS proxy
install_opener(build_opener(ProxyHandler({})))

KEY_FILE = os.environ.get(
    "GENIMG_KEY_FILE",
    os.path.join(os.environ.get("USERPROFILE", os.path.expanduser("~")),
                 "Desktop", "openai_api.txt"),
)
BASE_URL = os.environ.get("GENIMG_BASE_URL", "https://api.<your-proxy>.com").rstrip("/")
TIMEOUT = 300
...
def main():
    if len(sys.argv) < 3:
        print("usage: genimg.py <out.png> '<prompt>' [size] [model]")
        sys.exit(2)
    ...
    if "b64_json" in data:
        ...
        raise SystemExit(f"unexpected response shape: {body}")
